...
The brand mark is `fe/src/components/layout/BrandMark.tsx` — two overlapping rounded squares (an original "stacked photos" glyph, no lettering), rendered inline so it's crisp at any size. The identical shape is duplicated by hand in `fe/src/app/icon.svg` (Next.js App Router auto-serves `app/icon.svg` as the favicon — confirmed at `/icon.svg` after a production build) since Next's icon route can't import a React component; if `BrandMark.tsx` ever changes, update `icon.svg` to match. An earlier proposed logo (a modified Domino's Pizza mark) was declined and replaced with this original design — don't reintroduce a real company's trademark here.
